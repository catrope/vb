...
public class ProgramFuncDef extends FuncDef {
	LinkedList<String> varlist;
	Value val;
	Context evalContext;
	ConcreteValue mem;
	
	public ProgramFuncDef(LinkedList<String> varlist, Value val, Context evalContext) {
	...
	public ProgramFuncDef(LinkedList<String> varlist, Value val) {
	...
	public ConcreteValue evaluate(Context context, LinkedList<Value> vars) {		
		...
		if (vars.size() > 0) {
			...
			for(int i=0; i<varlist.size(); i++) {
				newContext.putFuncDef(varNames.next(), new ProgramFuncDef(new LinkedList<String>(), varValues.next(), myContext));
			}
